...
int HandleNewConnection(int RemoteSock)
{
	...
	switch(ReceivedBaseHeader.PacketType)
		...
		//-- if the established new connection is the case of BA
			...
			printf("public key of other side %s\n",assoc_req.PubKey);
...
//                                    BIGNUM *bin=NULL;
  ...
  //                                  if ((bin=BN_new()) == NULL) printf("error\n");
                                ...
                                HisPublic=EC_POINT_bn2point(group,bin,NULL,ctx);
                                if(HisPublic==NULL)
                                {
                                ...
                                aout=ECDH_compute_key(abuf,100,HisPublic,MyKey,KDF1_SHA11);
                                for (ii=0; ii<aout; ii++)
                                {
                                    sprintf(mySessionKey,"%02X",abuf[ii]);
				    strcat(mySessionKey1,mySessionKey);
                                    //BIO_puts(out,mySessionKey);
                                }


                                BIO_puts(out,mySessionKey1);
                                   BIO_puts(out,"\n");
	...
	if (MyX) BN_free(MyX);
	if (MyY) BN_free(MyY);
//	if (HisX) BN_free(HisX);
//	if (HisY) BN_free(HisY);
//	if (bin) BN_free(bin);
	if (MyKey) EC_KEY_free(MyKey);
...
assoc_node->MyKey=MyKey;
assoc_node->group=group;

strncpy(assoc_node->sessionKey,mySessionKey1,40);
